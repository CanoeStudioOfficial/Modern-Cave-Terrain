package com.lonelyxiya.minecraft.moderncaveterrain.config.io;
...
import com.lonelyxiya.minecraft.moderncaveterrain.enums.*;
...
/**
 * Utility class for populating a Modern Cave Terrain ConfigHolder from a config file.
 */
public class ConfigLoader {
     ...
     * The file must be located at {@code <configdirectory>/moderncaveterrain-1_12_2/DIM<id>_config.cfg}
    ...
    public static ConfigHolder loadConfigFromFileForDimension(int dimensionID) {
        ...
        File configFile = new File(ModernCaveTerrain.customConfigDir, fileName);

        if (!configFile.exists() || configFile.isDirectory()) {
            ModernCaveTerrain.LOGGER.info(String.format("Modern Cave Terrain config file for dimension %d not found. Using global config...", dimensionID));
            return new ConfigHolder().sanitize();
        }

        if (!configFile.canRead()) {
            ModernCaveTerrain.LOGGER.warn(String.format("Modern Cave Terrain config file for dimension %d not readable. Using global config...", dimensionID));
            return new ConfigHolder().sanitize();
        }

        ModernCaveTerrain.LOGGER.info(String.format("Reading Modern Cave Terrain config from file for dimension %d...", dimensionID));
     ...
     *         use the respective value in the global Modern Cave Terrain config
     */
    private static ConfigHolder parseConfigFromFile(File file) {
        ...
        try {
                    ...
                    if (Character.isLetterOrDigit(line.charAt(i)) || ALLOWED_CHARS.indexOf(line.charAt(i)) != -1 || (quoted && line.charAt(i) != '"')) {
                        ...
                        switch (line.charAt(i)) {
                            ...
                            case '=':
                                ...
                                if (!isTypeSpecified) {
                                    ModernCaveTerrain.LOGGER.warn(String.format("Error in Modern Cave Terrain config for %s (line %d): missing variable type specifier. Inferring String...", fileName, lineNum));
                                }
                                ...
                                if (target != null) {
                                    switch(type) {
                                        case INTEGER:
                                            if (!(target.type.equals(Integer.TYPE) || target.type.equals(Integer.class))) {
                                                ModernCaveTerrain.LOGGER.error(String.format("ERROR: WRONG TYPE for %s in config %s. Skipping...", fullName, fileName));
                                                i = line.length();
                                                continue;
                                            }
                                    ...
                                    ModernCaveTerrain.LOGGER.error(String.format("ERROR: INVALID PROPERTY %s in config %s. Skipping...", fullName, fileName));
                                    i = line.length();
                                    continue;
                                }

                                ModernCaveTerrain.LOGGER.debug(String.format("%s: overriding config option: %s", fileName, fullName));
            ...
            ModernCaveTerrain.LOGGER.error(String.format("ERROR LOADING Modern Cave Terrain CONFIG %s: %s.", fileName, e));
            ModernCaveTerrain.LOGGER.info("USING GLOBAL CONFIG FILE INSTEAD...");
            return new ConfigHolder().sanitize();
        } finally {
