 ...
 * This class serves as an interface between Modern Cave Terrain and FastNoise.
 */
public class NoiseGen {
    ...
    public ColumnNoiseBuffer interpolateNoiseColumns(ColumnNoiseBuffer buffer, int startX, int endX, int startZ, int endZ,
        ...
        int verticalStep = Math.max(1, ModernCaveTerrainSettings.NOISE_VERTICAL_SAMPLE_STEP);
        ...
        for (int columnSlot = 0; columnSlot < activeColumnCount; columnSlot++) {
            ...
            int noiseX = columnIndex / ModernCaveTerrainSettings.SUB_CHUNK_SIZE;
            int noiseZ = columnIndex % ModernCaveTerrainSettings.SUB_CHUNK_SIZE;
            float startCoeffX = ModernCaveTerrainSettings.START_COEFFS[noiseX];
            float endCoeffX = ModernCaveTerrainSettings.END_COEFFS[noiseX];
            float startCoeffZ = ModernCaveTerrainSettings.START_COEFFS[noiseZ];
            float endCoeffZ = ModernCaveTerrainSettings.END_COEFFS[noiseZ];
