...
/**
 * Per-chunk data shared by all Modern Cave Terrain carving passes.
 */
public class ChunkCaveContext {
    ...
    public ChunkCaveContext(World world, ChunkPrimer primer, int chunkX, int chunkZ, ConfigHolder config,
        ...
        this.oceanMask = needsOceanMask
            ? ModernCaveTerrainUtils.getOceanMaskFlat(world, chunkX, chunkZ, OCEAN_MASK_BORDER)
            : null;
    ...
    private static int[] buildSurfaceAltitudes(ChunkPrimer primer, boolean overrideSurfaceDetection, int surfaceSearchTopY) {
        ...
        int subChunkCount = CHUNK_SIZE / ModernCaveTerrainSettings.SUB_CHUNK_SIZE;
                ...
                for (int offsetX = 0; offsetX < ModernCaveTerrainSettings.SUB_CHUNK_SIZE; offsetX++) {
                    for (int offsetZ = 0; offsetZ < ModernCaveTerrainSettings.SUB_CHUNK_SIZE; offsetZ++) {
                        ...
                        surfaceAltitudes[columnIndex(localX, localZ)] = overrideSurfaceDetection
                            ? 1
                            : ModernCaveTerrainUtils.searchSurfaceAltitudeInRangeForColumn(primer, localX, localZ, surfaceSearchTopY, 0);
                    }
                }
            }
        }
            ...
            for (int localZ = 0; localZ < CHUNK_SIZE; localZ++) {
                ...
                if (biomeDistanceFactors != null) {
                    biomeDistanceFactors[columnIndex] =
                        ModernCaveTerrainUtils.biomeDistanceFactor(localX, localZ, OCEAN_MASK_BORDER, oceanMask, oceanMaskWidth, !oceanColumn);
                }
            }
        }
    }
