...
import com.lonelyxiya.minecraft.moderncaveterrain.world.bedrock.FlattenBedrock;
...
/**
 * Class that overrides vanilla cave gen with Modern Cave Terrain gen.
 * Combines multiple types of caves and caverns using different types of noise to create a
 * novel underground experience.
 */
public class MapGenModernCaveTerrain extends MapGenCaves {
    ...
    public MapGenModernCaveTerrain(InitMapGenEvent event) {
        this.defaultCaveGen = event.getOriginalGen();
    }

    /**
     * Function for generating Modern Cave Terrain in a single chunk. This overrides the vanilla cave generation, which is
    ...
    public synchronized void generate(World worldIn, int chunkX, int chunkZ, @Nonnull ChunkPrimer primer) {
        ...
        ModernCaveTerrainAPI.decorateCaves(worldIn, primer, chunkX, chunkZ, config);
    }

    /**
     * Initialize Modern Cave Terrain carvers and controllers for this dimension.
     * @param worldIn The minecraft world
     */
