...
import com.lonelyxiya.minecraft.moderncaveterrain.noise.NoiseUtils;
...
public class WaterRegionController {
    ...
    private IBlockState getLavaBlockFromString(String lavaString) {
        ...
        try {
            ...
            ModernCaveTerrain.LOGGER.info("Using block '" + lavaString + "' as lava in cave generation for dimension " +
                    ModernCaveTerrainUtils.dimensionAsString(dimensionID, dimensionName) + " ...");
        } catch (Exception e) {
            ModernCaveTerrain.LOGGER.warn("Unable to use block '" + lavaString + "': " + e);
            ModernCaveTerrain.LOGGER.warn("Using vanilla lava instead...");
            lavaBlock = Blocks.LAVA.getDefaultState();
        }
        if (lavaBlock == null) {
            ModernCaveTerrain.LOGGER.warn("Unable to use block '" + lavaString + "': null block returned.\n Using vanilla lava instead...");
            lavaBlock = Blocks.LAVA.getDefaultState();
        }
        return lavaBlock;
    }
    ...
    private IBlockState getWaterBlockFromString(String waterString) {
        ...
        try {
            ...
            ModernCaveTerrain.LOGGER.info("Using block '" + waterString + "' as water in cave generation for dimension " +
                    ModernCaveTerrainUtils.dimensionAsString(dimensionID, dimensionName) + " ...");
        } catch (Exception e) {
            ModernCaveTerrain.LOGGER.warn("Unable to use block '" + waterString + "': " + e);
            ModernCaveTerrain.LOGGER.warn("Using vanilla water instead...");
            waterBlock = Blocks.WATER.getDefaultState();
        }

        if (waterBlock == null) {
            ModernCaveTerrain.LOGGER.warn("Unable to use block '" + waterString + "': null block returned.\n Using vanilla water instead...");
            waterBlock = Blocks.WATER.getDefaultState();
        }
