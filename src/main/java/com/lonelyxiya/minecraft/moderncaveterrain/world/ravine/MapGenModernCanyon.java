package com.lonelyxiya.minecraft.moderncaveterrain.world.ravine;
 ...
 */
public class MapGenModernCanyon extends MapGenRavine {
    ...
    public MapGenModernCanyon(InitMapGenEvent event) {
    ...
    protected void digBlock(ChunkPrimer primer, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop) {
        int localX = ModernCaveTerrainUtils.getLocal(x);
        int localZ = ModernCaveTerrainUtils.getLocal(z);
    ...
    private void prepareChunkContext(int chunkX, int chunkZ) {
        if (currChunkLiquidBlocks == null || chunkX != currChunkX || chunkZ != currChunkZ) {
            ...
            if (config.enableFloodedRavines.get()) {
                currChunkOceanMask = ModernCaveTerrainUtils.getOceanMaskFlat(world, chunkX, chunkZ, OCEAN_MASK_BORDER);
    ...
    private float[] buildFloodFactors(boolean[] oceanMask, int oceanMaskWidth) {
        ...
        for (int localX = 0; localX < 16; localX++) {
            for (int localZ = 0; localZ < 16; localZ++) {
                factors[localX * 16 + localZ] = ModernCaveTerrainUtils.biomeDistanceFactor(localX, localZ, OCEAN_MASK_BORDER, oceanMask, oceanMaskWidth, false);
            }
        }
        return factors;
    }
